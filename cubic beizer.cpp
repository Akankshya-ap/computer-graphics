...
void drawSq(int x[4], int y[4])
{
	...
	{
		ui=j/100.0;
		xf=(double)(1-ui*ui*ui)*(double)x[0]+3*(double)(1-ui)*(1-ui)*ui*(double)x[1]+3*ui*ui*(i-ui)*(double)x[2]+ui*ui*ui*x[3];
		yf=(double)(1-ui*ui*ui)*(double)y[0]+3*(double)(1-ui)*(1-ui)*ui*(double)y[1]+3*ui*ui*(i-ui)*(double)y[2]+ui*ui*ui*y[3];
		printf("%f %f ",xf,yf);
		draw_pixel(xf,yf);
	}
}
